...
export class BaseEditViewModel<T extends IBaseItem> extends WorkViewModel {
    ...
    public canActivate(params?: any, config?: any, instruction?: any): any {
        let px = this.person;
        return (px !== null) && (px.is_super || px.is_admin || px.is_prof);
    }// activate
    //
    ...
    public set items(s: T[]) {
        this._items = InfoRoot.check_array(s);
    }
    //
    ...
    protected set allIds(s: string[]) {
        this._data_ids = InfoRoot.check_array(s);
    }
    //
    ...
    protected set oldItem(s: T) {
        this._old_item = InfoRoot.check_value(s);
    }
    ...
    public refresh(): Promise<any> {
        ...
        return this.dataService.get_items(model, startKey, endKey).then((rr) => {
            let rx = InfoRoot.check_array(rr);
            if (self.hasAvatars) {
            ...
            self.items = InfoRoot.check_array(dd);
    ...
    public refreshAll(): Promise<any> {
        ...
        return this.dataService.get_ids(startKey, endKey).then((ids) => {
            self.allIds = InfoRoot.check_array(ids);
